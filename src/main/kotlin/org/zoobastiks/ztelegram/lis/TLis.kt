...
class TLis(private val plugin: ZTele) : Listener {
    ...
    fun onPlayerDeath(event: PlayerDeathEvent) {
        ...
        // Получаем сообщение о смерти и преобразуем его в обычный текст
        var deathMessage = ""
        
        try {
            // Пробуем получить компонентное сообщение (для Paper)
            val deathComponent = event.deathMessage()
            if (deathComponent != null) {
                // Преобразуем компонент в обычный текст
                deathMessage = PlainTextComponentSerializer.plainText().serialize(deathComponent)
            } else {
                // Запасной вариант для старых версий или если компонент null
                deathMessage = event.deathMessage ?: ""
            }
        } catch (e: Exception) {
            // Запасной вариант для Spigot/Bukkit
            deathMessage = event.deathMessage ?: ""
            plugin.logger.warning("Failed to get death message component: ${e.message}")
        }
        
        // Логируем оригинальное сообщение о смерти для отладки
        plugin.logger.info("Original death message for ${player.name}: '$deathMessage'")
