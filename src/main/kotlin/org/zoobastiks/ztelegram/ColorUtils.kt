...
object ColorUtils {
    ...
    fun stripColorCodes(text: String): String {
        ...
        // Удаляем цветовые коды (§ и &)
        var result = processedText.replace("§[0-9a-fk-orA-FK-OR]".toRegex(), "")
        result = result.replace("&[0-9a-fk-orA-FK-OR]".toRegex(), "")
        
        // Удаляем hex коды
        result = result.replace("§x(§[0-9a-fA-F]){6}".toRegex(), "")
